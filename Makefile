...
CXX=g++
PLATFORM_LDFLAGS= -lpthread -lrt -lsnappy
...
ifndef SHANNONDB_PATH
SHANNONDB_PATH = $(THIRD_PATH)/kv_libc++
endif
SHANNONDB = $(SHANNONDB_PATH)/libshannondb_cxx$(DEBUG_SUFFIX).a
...
INCLUDE_PATH = -I. \
							 ...
							 -I$(BLACKWIDOW_PATH)/src 	  \
							 -I$(SHANNONDB_PATH)/include  \
							 -I$(SHANNONDB_PATH)
							 #-I$(ROCKSDB_PATH) \
...
#$(BINARY): $(SLASH) $(SHANNONDB) $(PINK) $(ROCKSDB) $(BLACKWIDOW) $(GLOG) $(LIBOBJECTS)
$(BINARY): $(SLASH) $(SHANNONDB) $(PINK) $(BLACKWIDOW) $(GLOG) $(LIBOBJECTS)
	$(AM_V_at)rm -f $@
	$(AM_V_at)$(AM_LINK)
...
blackwidow_examples:
	$(AM_V_at)make -C $(BLACKWIDOW_PATH)/examples ROCKSDB_PATH=$(ROCKSDB_PATH) SLASH_PATH=$(SLASH_PATH) DEBUG_LEVEL=$(DEBUG_LEVEL)
