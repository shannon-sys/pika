...
void GeoPosCmd::Do() {
  ...
  for (const auto& member : members_) {
    shannon::Status s = g_pika_server->db()->ZScore(key_, member, &score);
    if (s.ok()) {
      double xy[2];
...
void GeoDistCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->ZScore(key_, first_pos_, &first_score);
  if (s.ok()) {
...
void GeoRadiusByMemberCmd::Do() {
  double score;
  shannon::Status s = g_pika_server->db()->ZScore(key_, range_.member, &score);
  if (s.ok()) {
    double xy[2];
    GeoHashBits hash = { .bits = (uint64_t)score, .step = GEO_STEP_MAX };
    geohashDecodeToLongLatWGS84(hash, xy);
