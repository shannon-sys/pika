...
void SAddCmd::Do() {
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->SAdd(key_, members_, &count);
  if (!s.ok()) {
...
void SPopCmd::Do() {
  std::string member;
  shannon::Status s = g_pika_server->db()->SPop(key_, &member);
  if (s.ok()) {
...
void SCardCmd::Do() {
  int32_t card = 0;
  shannon::Status s = g_pika_server->db()->SCard(key_, &card);
...
void SMembersCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->SMembers(key_, &members);
  if (s.ok() || s.IsNotFound()) {
...
void SScanCmd::Do() {
  ...
  std::vector<std::string> members;
  shannon::Status s = g_pika_server->db()->SScan(key_, cursor_, pattern_, count_, &members, &next_cursor);
...
void SRemCmd::Do() {
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->SRem(key_, members_, &count);
  res_.AppendInteger(count);
  return;
}
...
void SUnionstoreCmd::Do() {
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->SUnionstore(dest_key_, keys_, &count);
  if (s.ok()) {
    res_.AppendInteger(count);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void SInterstoreCmd::Do() {
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->SInterstore(dest_key_, keys_, &count);
  if (s.ok()) {
    res_.AppendInteger(count);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void SDiffstoreCmd::Do() {
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->SDiffstore(dest_key_, keys_, &count);
  if (s.ok()) {
    res_.AppendInteger(count);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void SMoveCmd::Do() {
  int32_t res = 0;
  shannon::Status s = g_pika_server->db()->SMove(src_key_, dest_key_, member_, &res);
  if (s.ok() || s.IsNotFound()) {
    if (s.IsNotFound()){
...
void SRandmemberCmd::Do() {
  std::vector<std::string> members;
  shannon::Status s = g_pika_server->db()->SRandmember(key_, count_, &members);
  if (s.ok() || s.IsNotFound()) {
