...
void ZAddCmd::Do() {
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->ZAdd(key_, score_members, &count);
  if (s.ok()) {
...
void ZCardCmd::Do() {
  int32_t card = 0;
  shannon::Status s = g_pika_server->db()->ZCard(key_, &card);
...
void ZScanCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->ZScan(key_, cursor_, pattern_, count_, &score_members, &next_cursor);
  if (s.ok() || s.IsNotFound()) {
...
void ZIncrbyCmd::Do() {
  double score = 0;
  shannon::Status s = g_pika_server->db()->ZIncrby(key_, member_, by_, &score);
  if (s.ok()) {
    char buf[32];
...
void ZRangeCmd::Do() {
  std::vector<blackwidow::ScoreMember> score_members;
  shannon::Status s = g_pika_server->db()->ZRange(key_, start_, stop_, &score_members);
  if (s.ok() || s.IsNotFound()) {
    if (is_ws_) {
      char buf[32];
      int64_t len;
...
void ZRevrangeCmd::DoInitial(PikaCmdArgsType &argv, const CmdInfo* const ptr_info) {
  ...
  ZsetRangeParentCmd::DoInitial(argv, NULL);
  int32_t card = 0;
  shannon::Status status = g_pika_server->db()->ZCard(key_, &card);
...
void ZRevrangeCmd::Do() {
  std::vector<blackwidow::ScoreMember> score_members;
  shannon::Status s = g_pika_server->db()->ZRevrange(key_, start_, stop_, &score_members);
  if (s.ok() || s.IsNotFound()) {
    if (is_ws_) {
      char buf[32];
      int64_t len;
...
void ZRangebyscoreCmd::Do() {
  ...
  std::vector<blackwidow::ScoreMember> score_members;
  shannon::Status s = g_pika_server->db()->ZRangebyscore(key_, min_score_, max_score_, left_close_, right_close_, &score_members);
  if (!s.ok() && !s.IsNotFound()) {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
    return;
  }
...
void ZRevrangebyscoreCmd::Do() {
  ...
  std::vector<blackwidow::ScoreMember> score_members;
  shannon::Status s = g_pika_server->db()->ZRevrangebyscore(key_, min_score_, max_score_, left_close_, right_close_, &score_members);
  if (!s.ok() && !s.IsNotFound()) {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
    return;
  }
...
void ZCountCmd::Do() {
  ...
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->ZCount(key_, min_score_, max_score_, left_close_, right_close_, &count);
  if (s.ok() || s.IsNotFound()) {
    res_.AppendInteger(count);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void ZRemCmd::Do() {
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->ZRem(key_, members_, &count);
  if (s.ok() || s.IsNotFound()) {
    res_.AppendInteger(count);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void ZUnionstoreCmd::Do() {
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->ZUnionstore(dest_key_, keys_, weights_, aggregate_, &count);
  if (s.ok()) {
    res_.AppendInteger(count);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void ZInterstoreCmd::Do() {
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->ZInterstore(dest_key_, keys_, weights_, aggregate_, &count);
  if (s.ok()) {
    res_.AppendInteger(count);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void ZRankCmd::Do() {
  int32_t rank = 0;
  shannon::Status s = g_pika_server->db()->ZRank(key_, member_, &rank);
  if (s.ok()) {
...
void ZRevrankCmd::Do() {
  int32_t revrank = 0;
  shannon::Status s = g_pika_server->db()->ZRevrank(key_, member_, &revrank);
  if (s.ok()) {
...
void ZScoreCmd::Do() {
  double score = 0;
  shannon::Status s = g_pika_server->db()->ZScore(key_, member_, &score);
  if (s.ok()) {
    char buf[32];
    int64_t len = slash::d2string(buf, sizeof(buf), score);
    res_.AppendStringLen(len);
...
void ZRangebylexCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->ZRangebylex(key_, min_member_, max_member_, left_close_, right_close_, &members);
  if (!s.ok() && !s.IsNotFound()) {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
    return;
  }
...
void ZLexcountCmd::Do() {
  ...
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->ZLexcount(key_, min_member_, max_member_, left_close_, right_close_, &count);
  if (!s.ok() && !s.IsNotFound()) {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
    return;
  }
...
void ZRemrangebyrankCmd::Do() {
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->ZRemrangebyrank(key_, start_rank_, stop_rank_, &count);
  if (s.ok() || s.IsNotFound()) {
    res_.AppendInteger(count);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void ZRemrangebyscoreCmd::Do() {
  ...
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->ZRemrangebyscore(key_, min_score_, max_score_, left_close_, right_close_, &count);
  if (!s.ok() && !s.IsNotFound()) {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
    return;
  }
...
void ZRemrangebylexCmd::Do() {
  ...
  int32_t count = 0;
  shannon::Status s = g_pika_server->db()->ZRemrangebylex(key_, min_member_, max_member_, left_close_, right_close_, &count);
  if (!s.ok() && !s.IsNotFound()) {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
    return;
  }
