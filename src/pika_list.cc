...
void LIndexCmd::Do() {
  std::string value;
  shannon::Status s = g_pika_server->db()->LIndex(key_, index_, &value);
  if (s.ok()) {
...
void LInsertCmd::Do() {
  int64_t llen = 0;
  shannon::Status s = g_pika_server->db()->LInsert(key_, dir_, pivot_, value_, &llen);
...
void LLenCmd::Do() {
  uint64_t llen = 0;
  shannon::Status s = g_pika_server->db()->LLen(key_, &llen);
  if (s.ok() || s.IsNotFound()){
    res_.AppendInteger(llen);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void LPushCmd::Do() {
  uint64_t llen = 0;
  shannon::Status s = g_pika_server->db()->LPush(key_, values_, &llen);
  if (s.ok()) {
    res_.AppendInteger(llen);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void LPopCmd::Do() {
  std::string value;
  shannon::Status s = g_pika_server->db()->LPop(key_, &value);
  if (s.ok()) {
    res_.AppendString(value);
  } else if (s.IsNotFound()) {
    res_.AppendStringLen(-1);
...
void LPushxCmd::Do() {
  uint64_t llen = 0;
  shannon::Status s = g_pika_server->db()->LPushx(key_, value_, &llen);
  if (s.ok() || s.IsNotFound()) {
    res_.AppendInteger(llen);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void LRangeCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->LRange(key_, left_, right_, &values);
  if (s.ok()) {
...
void LRemCmd::Do() {
  uint64_t res = 0;
  shannon::Status s = g_pika_server->db()->LRem(key_, count_, value_, &res);
  if (s.ok() || s.IsNotFound()) {
...
void LSetCmd::Do() {
    shannon::Status s = g_pika_server->db()->LSet(key_, index_, value_);
    if (s.ok()) {
...
void LTrimCmd::Do() {
  shannon::Status s = g_pika_server->db()->LTrim(key_, start_, stop_);
  if (s.ok() || s.IsNotFound()) {
    res_.SetRes(CmdRes::kOk);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void RPopCmd::Do() {
  std::string value;
  shannon::Status s = g_pika_server->db()->RPop(key_, &value);
  if (s.ok()) {
    res_.AppendString(value);
  } else if (s.IsNotFound()) {
    res_.AppendStringLen(-1);
...
void RPopLPushCmd::Do() {
  std::string value;
  shannon::Status s = g_pika_server->db()->RPoplpush(source_, receiver_, &value);
  if (s.ok()) {
    res_.AppendString(value);
  } else if (s.IsNotFound()) {
    res_.AppendStringLen(-1);
...
void RPushCmd::Do() {
  uint64_t llen = 0;
  shannon::Status s = g_pika_server->db()->RPush(key_, values_, &llen);
  if (s.ok()) {
    res_.AppendInteger(llen);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void RPushxCmd::Do() {
  uint64_t llen = 0;
  shannon::Status s = g_pika_server->db()->RPushx(key_, value_, &llen);
  if (s.ok() || s.IsNotFound()) {
    res_.AppendInteger(llen);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
