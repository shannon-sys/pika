...
void InfoCmd::InfoData(std::string &info) {
  ...
  // shannon related memory usage
  uint64_t memtable_usage = 0, table_reader_usage = 0;
  g_pika_server->db()->GetUsage(blackwidow::USAGE_TYPE_shannon_MEMTABLE, &memtable_usage);
  g_pika_server->db()->GetUsage(blackwidow::USAGE_TYPE_shannon_TABLE_READER, &table_reader_usage);
