...
void HDelCmd::Do() {
  int32_t num = 0;
  shannon::Status s = g_pika_server->db()->HDel(key_, fields_, &num);
...
void HSetCmd::Do() {
  int32_t ret = 0;
  shannon::Status s = g_pika_server->db()->HSet(key_, field_, value_, &ret);
  if (s.ok()) {
...
void HGetCmd::Do() {
  std::string value;
  shannon::Status s = g_pika_server->db()->HGet(key_, field_, &value);
  if (s.ok()) {
...
void HGetallCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->HGetall(key_, &fvs);
  if (s.ok() || s.IsNotFound()) {
...
void HExistsCmd::Do() {
  shannon::Status s = g_pika_server->db()->HExists(key_, field_);
  if (s.ok()) {
    res_.AppendContent(":1");
  } else if (s.IsNotFound()) {
...
void HIncrbyCmd::Do() {
  int64_t new_value;
  shannon::Status s = g_pika_server->db()->HIncrby(key_, field_, by_, &new_value);
  if (s.ok() || s.IsNotFound()) {
...
void HIncrbyfloatCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->HIncrbyfloat(key_, field_, by_, &new_value);
  if (s.ok()) {
...
void HKeysCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->HKeys(key_, &fields);
  if (s.ok() || s.IsNotFound()) {
...
void HLenCmd::Do() {
  int32_t len = 0;
  shannon::Status s = g_pika_server->db()->HLen(key_, &len);
  if (s.ok() || s.IsNotFound()) {
...
void HMgetCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->HMGet(key_, fields_, &vss);
  if (s.ok() || s.IsNotFound()) {
...
void HMsetCmd::Do() {
  shannon::Status s = g_pika_server->db()->HMSet(key_, fvs_);
  if (s.ok()) {
...
void HSetnxCmd::Do() {
  int32_t ret = 0;
  shannon::Status s = g_pika_server->db()->HSetnx(key_, field_, value_, &ret);
  if (s.ok()) {
    res_.AppendContent(":" + std::to_string(ret));
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void HStrlenCmd::Do() {
  int32_t len = 0;
  shannon::Status s = g_pika_server->db()->HStrlen(key_, field_, &len);
  if (s.ok() || s.IsNotFound()) {
    res_.AppendInteger(len);
  } else {
...
void HValsCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->HVals(key_, &values);
  if (s.ok() || s.IsNotFound()) {
...
void HScanCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->HScan(key_, cursor_, pattern_, count_, &field_values, &next_cursor);
...
void HScanxCmd::Do() {
  ...
  std::vector<blackwidow::FieldValue> field_values;
  shannon::Status s = g_pika_server->db()->HScanx(key_, start_field_, pattern_, count_, &field_values, &next_field);
...
void PKHScanRangeCmd::Do() {
  std::string next_field;
  std::vector<blackwidow::FieldValue> field_values;
  shannon::Status s = g_pika_server->db()->PKHScanRange(key_, field_start_, field_end_,
...
void PKHRScanRangeCmd::Do() {
  std::string next_field;
  std::vector<blackwidow::FieldValue> field_values;
  shannon::Status s = g_pika_server->db()->PKHRScanRange(key_, field_start_, field_end_,
          pattern_, limit_, &field_values, &next_field);
