...
void GetCmd::Do() {
  std::string value;
  shannon::Status s = g_pika_server->db()->Get(key_, &value);
  if (s.ok()) {
...
void IncrCmd::Do() {
  shannon::Status s = g_pika_server->db()->Incrby(key_, 1, &new_value_);
  if (s.ok()) {
...
void IncrbyCmd::Do() {
  shannon::Status s = g_pika_server->db()->Incrby(key_, by_, &new_value_);
  if (s.ok()) {
    res_.AppendContent(":" + std::to_string(new_value_));
  } else if (s.IsCorruption() && s.ToString() == "Corruption: Value is not a integer") {
    res_.SetRes(CmdRes::kInvalidInt);
...
void IncrbyfloatCmd::Do() {
  shannon::Status s = g_pika_server->db()->Incrbyfloat(key_, value_, &new_value_);
  if (s.ok()) {
...
void DecrCmd::Do() {
  shannon::Status s = g_pika_server->db()->Decrby(key_, 1, &new_value_);
  if (s.ok()) {
   res_.AppendContent(":" + std::to_string(new_value_));
  } else if (s.IsCorruption() && s.ToString() == "Corruption: Value is not a integer") {
    res_.SetRes(CmdRes::kInvalidInt);
...
void DecrbyCmd::Do() {
  shannon::Status s = g_pika_server->db()->Decrby(key_, by_, &new_value_);
  if (s.ok()) {
    res_.AppendContent(":" + std::to_string(new_value_));
  } else if (s.IsCorruption() && s.ToString() == "Corruption: Value is not a integer") {
    res_.SetRes(CmdRes::kInvalidInt);
...
void GetsetCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->GetSet(key_, new_value_, &old_value);
  if (s.ok()) {
    if (old_value.empty()) {
...
void AppendCmd::Do() {
  int32_t new_len = 0;
  shannon::Status s = g_pika_server->db()->Append(key_, value_, &new_len);
  if (s.ok() || s.IsNotFound()) {
...
void MgetCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->MGet(keys_, &vss);
  if (s.ok()) {
...
void KeysCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->Keys(type_, pattern_, &keys);
...
void SetnxCmd::Do() {
  success_ = 0;
  shannon::Status s = g_pika_server->db()->Setnx(key_, value_, &success_);
  if (s.ok()) {
    res_.AppendInteger(success_);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void SetexCmd::Do() {
  shannon::Status s = g_pika_server->db()->Setex(key_, value_, sec_);
  if (s.ok()) {
    res_.SetRes(CmdRes::kOk);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void PsetexCmd::Do() {
  shannon::Status s = g_pika_server->db()->Setex(key_, value_, usec_ / 1000);
  if (s.ok()) {
    res_.SetRes(CmdRes::kOk);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void DelvxCmd::Do() {
  shannon::Status s = g_pika_server->db()->Delvx(key_, value_, &success_);
  if (s.ok() || s.IsNotFound()) {
    res_.AppendInteger(success_);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void MsetnxCmd::Do() {
  success_ = 0;
  shannon::Status s = g_pika_server->db()->MSetnx(kvs_, &success_);
  if (s.ok()) {
    res_.AppendInteger(success_);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void GetrangeCmd::Do() {
  std::string substr;
  shannon::Status s = g_pika_server->db()->Getrange(key_, start_, end_, &substr);
  if (s.ok() || s.IsNotFound()) {
...
void SetrangeCmd::Do() {
  int32_t new_len;
  shannon::Status s = g_pika_server->db()->Setrange(key_, offset_, value_, &new_len);
  if (s.ok()) {
    res_.AppendInteger(new_len);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void StrlenCmd::Do() {
  int32_t len = 0;
  shannon::Status s = g_pika_server->db()->Strlen(key_, &len);
  if (s.ok() || s.IsNotFound()) {
...
void ExistsCmd::Do() {
  std::map<blackwidow::DataType, shannon::Status> type_status;
...
void TypeCmd::Do() {
  std::string res;
  shannon::Status s = g_pika_server->db()->Type(key_, &res);
  if (s.ok()) {
...
void ScanxCmd::Do() {
  std::string next_key;
  std::vector<std::string> keys;
  shannon::Status s = g_pika_server->db()->Scanx(type_, start_key_, pattern_, count_, &keys, &next_key);
...
void PKScanRangeCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->PKScanRange(type_, key_start_, key_end_, pattern_, limit_, &keys, &kvs, &next_key);
...
void PKRScanRangeCmd::Do() {
  ...
  std::vector<std::string> keys;
  std::vector<blackwidow::KeyValue> kvs;
  shannon::Status s = g_pika_server->db()->PKRScanRange(type_, key_start_, key_end_, pattern_, limit_, &keys, &kvs, &next_key);
