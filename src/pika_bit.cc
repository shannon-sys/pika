...
void BitSetCmd::Do() {
  std::string value;
  int32_t bit_val = 0;
  shannon::Status s = g_pika_server->db()->SetBit(key_, bit_offset_, on_, &bit_val);
  if (s.ok()){
...
void BitGetCmd::Do() {
  int32_t bit_val = 0;
  shannon::Status s = g_pika_server->db()->GetBit(key_, bit_offset_, &bit_val);
  if (s.ok()) {
    res_.AppendInteger((int)bit_val);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void BitOpCmd::Do() {
  ...
  shannon::Status s = g_pika_server->db()->BitOp(op_, dest_key_, src_keys_, &result_length);
  if (s.ok()) {
