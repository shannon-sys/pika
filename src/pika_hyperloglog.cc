...
void PfAddCmd::Do() {
  bool update = false;
  shannon::Status s = g_pika_server->db()->PfAdd(key_, values_, &update);
  if (s.ok() && update) {
...
void PfCountCmd::Do() {
  int64_t value_ = 0;
  shannon::Status s = g_pika_server->db()->PfCount(keys_, &value_);
  if (s.ok()) {
...
void PfMergeCmd::Do() {
  shannon::Status s = g_pika_server->db()->PfMerge(keys_);
  if (s.ok()) {
