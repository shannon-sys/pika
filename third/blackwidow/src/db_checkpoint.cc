...
#if (shannon_MAJOR < 5 || (shannon_MAJOR == 5 && shannon_MINOR < 3))
...
class DBCheckpointImpl : public DBCheckpoint {
 public:
  ...
  // Builds an openable snapshot of shannon on the same disk, which
...
// Builds an openable snapshot of shannon
...
Status DBCheckpointImpl::CreateCheckpointWithFiles(const std::string& checkpoint_dir,
  ...
  //Log(db_->GetOptions().info_log, "Number of log files %" shannon_PRIszt, live_wal_files.size());
