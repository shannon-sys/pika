...
Status BackupEngine::NewCheckpoint(shannon::DB* shannon_db, const std::string& type) {
  shannon::DBCheckpoint* checkpoint;
  Status s = shannon::DBCheckpoint::Create(shannon_db, &checkpoint);
  if (!s.ok()) {
...
Status BackupEngine::Open(blackwidow::BlackWidow *blackwidow,
  ...
  *backup_engine_ptr = new BackupEngine();
  ...
  shannon::Status s;
  shannon::DB *shannon_db;
  std::string types[] = {STRINGS_DB, HASHES_DB, LISTS_DB, ZSETS_DB, SETS_DB};
  for (const auto& type : types) {
    if ((shannon_db = blackwidow->GetDBByType(type)) == NULL) {
      s = Status::Corruption("Error db type");
    }

    if (s.ok()) {
      s = (*backup_engine_ptr)->NewCheckpoint(shannon_db, type);
    }
...
Status BackupEngine::CreateNewBackupSpecify(const std::string &backup_dir, const std::string &type) {
  std::map<std::string, shannon::DBCheckpoint*>::iterator it_engine = engines_.find(type);
