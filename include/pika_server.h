...
class PikaServer {
 public:
  ...
  struct KeyScanInfo {
    time_t start_time;
    std::string s_start_time;
    std::vector<blackwidow::KeyInfo> key_infos; //the order is kv, hash, list, zset, set
    bool key_scaning_;
    KeyScanInfo() :
        start_time(0),
 ...
 private:
  ...
  shannon::Status DeleteBinlogFile(uint32_t pro_num);
  bool GetBinlogFiles(uint32_t *first_pro_num, uint32_t *last_pro_num);
